    ...
    for i in range(0, img.shape[0]):
        for j in range(0, img.shape[1]):
            grayscale_img[i][j] = 0.2989 * img[i][j][2] + \
                                  0.587 * img[i][j][1] + 0.114 * img[i][j][0]
    return grayscale_img


def find_face_local_mean_thresholding(image, neighbours, constant1, constant2):
    bounded_image = np.zeros((image.shape[0], image.shape[1]), dtype="uint8")

    n = int(neighbours / 2)
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            store = image[max(i - n, 0):min(i + n, image.shape[0]) + 1, max(j - n, 0):min(j + n, image.shape[1]) + 1].flatten()
            pixel = np.rint(np.sum(store) / store.shape[0])
            bounded_image[i][j] = 255 if (constant1 > pixel) else 0
...
binarised_image = find_face_local_mean_thresholding(image[180:, 200:500], 6, 120, 160)
...
#                         if (
#                                 image.shape[0] > ny >= 0 == image[ny, nx] and
#                                 image.shape[1] > nx >= 0 == labeled_image[ny, nx]
#                         ):
#                             stack.append((ny, nx))
#
#                 current_label += 1
